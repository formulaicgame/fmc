use super::components::{GlobalTransform, Transform, TransformTreeChanged};
...
/// the serial version.
mod parallel {
    use super::{GlobalTransform, Transform, TransformTreeChanged};
    // TODO: this implementation could be used in no_std if there are equivalents of these.
    use bevy::ecs::{entity::UniqueEntityIter, prelude::*, system::lifetimeless::Read};
    ...
    fn propagation_worker(queue: &WorkQueue, nodes: &NodeQuery) {
        let _span = bevy::log::info_span!("transform propagation worker").entered();
